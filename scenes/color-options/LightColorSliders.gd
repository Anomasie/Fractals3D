...
@onready var PreviewSets = [
	$Dialogue/Content/Lines/Columns/Frame/Margin/Previews/Set1,
	$Dialogue/Content/Lines/Columns/Frame/Margin/Previews/Set2,
	$Dialogue/Content/Lines/Columns/Frame/Margin/Previews/Set3
]
@onready var ActivePreview = PreviewSets[0]
...
func _ready():
	# connect
	## connect previews
	for i in len(PreviewSets):
		PreviewSets[i].pressed.connect(_on_preview_pressed.bind(i))
	...
	if not Engine.is_editor_hint():
		...
		Global.tooltip_nodes.append_array([
			HueSlider, ValueSlider, SatSlider,
...
func open(colors, type = OptionsLabel.text):
	if not type:
		OptionsLabel.hide()
	else:
...
func set_active_color(color = get_active_color()):
	disabled += 1
	
	# preview picture
	ActivePreview.modulate = color
	# update backgrounds
	## value slider
...
func set_colors(color_vector) -> void:
	for i in len(PreviewSets):
		PreviewSets[i].modulate = color_vector[i]
		if PreviewSets[i].button_pressed:
			set_active_color(color_vector[i])
...
func get_colors():
	return [
		PreviewSets[0].modulate,
		PreviewSets[1].modulate,
		PreviewSets[2].modulate
	]
...
func _on_preset_pressed(i):
	set_active_color(Presets[i].modulate)
	if disabled == 0:
		color_changed.emit()
		color_changed_vastly.emit()
...
func _on_user_preset_pressed(i):
	if UserPresets[i].visible:
		set_active_color(UserPresets[i].modulate)
		if disabled == 0:
			color_changed.emit()
			color_changed_vastly.emit()

func _on_add_button_pressed():
	Global.user_saved_colors.push_front(get_active_color())
...
func delete_hash_text():
	Hash.text = ""
	Hash.placeholder_text = ActivePreview.modulate.to_html(false)

func _on_hash_text_submitted(new_text):
	Hash.release_focus()
	if len(new_text) == 6:
		set_active_color(Color.from_string(new_text, ActivePreview.modulate))
		color_changed.emit()
	else:
		delete_hash_text()
...
func _on_preview_pressed(i) -> void:
	ActivePreview = PreviewSets[i]
	for preview in PreviewSets:
		preview.button_pressed = ( preview != ActivePreview )
	set_active_color( ActivePreview.modulate )

# random color

func _on_random_button_pressed() -> void:
	set_active_color(Color.from_hsv(randf(), randf(), randf()))
	color_changed.emit()
...
func reload_language():
	match Global.language:
		"GER":
			ActivePreview.tooltip_text = "aktuelle Farbe"
		...
		_:
			ActivePreview.tooltip_text = "preview of selected color"
